...
function getExt(filename, url) {
    ...
    const p = clean.split('.'); if (p.length > 1) return p.pop().toLowerCase();
    return '';
}
...
// ─────────────────────────────────────────────────────────────
// Core: send to Aria2 Streamer app
// ─────────────────────────────────────────────────────────────
async function sendToAria2(url, filename, referrer) {
    ...
    try {
        ...
        try {
            ...
            await chrome.tabs.create({ url: 'aria2streamer://open', active: false });
        } catch (e) {}
        ...
        try {
            ...
            const tabs = await chrome.tabs.query({ url: 'aria2streamer://*' });
